<template>
    <el-aside width="200px" style="background-color: rgb(238, 241, 246)">
        <el-menu :default-openeds="[]" @select="handleSelect">
          <h1>Element UI</h1>
          <el-submenu index="1">
            <template slot="title"><i class="el-icon-message"></i>导航一</template>
            <el-menu-item-group>
              <template slot="title">分组一</template>
              <el-menu-item index="1-1">首页</el-menu-item>
              <el-menu-item index="1-2">每日学习</el-menu-item>
            </el-menu-item-group>
            <el-menu-item-group title="分组2">
              <el-menu-item index="1-4">录入学生信息</el-menu-item>
              <el-menu-item index="1-3">查看学生信息</el-menu-item>
            </el-menu-item-group>
          </el-submenu>
          <!--<el-submenu index="2">-->
            <!--<template slot="title"><i class="el-icon-menu"></i>数据报表</template>-->
            <!--<el-menu-item-group>-->
              <!--<template slot="title">分组一</template>-->
              <!--<el-menu-item index="2-1">报表1</el-menu-item>-->
              <!--<el-menu-item index="2-2">报表2</el-menu-item>-->
            <!--</el-menu-item-group>-->
            <!--<el-menu-item-group title="分组2">-->
              <!--<el-menu-item index="2-3">选项3</el-menu-item>-->
            <!--</el-menu-item-group>-->
            <!--<el-submenu index="2-4">-->
              <!--<template slot="title">选项4</template>-->
              <!--<el-menu-item index="2-4-1">选项4-1</el-menu-item>-->
            <!--</el-submenu>-->
          <!--</el-submenu>-->
          <el-submenu index="3">
            <template slot="title"><i class="el-icon-setting"></i>导航三</template>
            <el-menu-item-group>
              <template slot="title">分组一</template>
              <el-menu-item index="3-1">Linux</el-menu-item>
              <el-menu-item index="3-2">选项2</el-menu-item>
            </el-menu-item-group>
            <el-menu-item-group title="分组2">
              <el-menu-item index="3-3">选项3</el-menu-item>
            </el-menu-item-group>
            <el-submenu index="3-4">
              <template slot="title">选项4</template>
              <el-menu-item index="3-4-1">选项4-1</el-menu-item>
            </el-submenu>
          </el-submenu>
          <el-submenu index="4">
            <template slot="title"><i class="el-icon-info"></i>客户信息管理</template>
              <el-menu-item index="4-1">新增客户信息</el-menu-item>
              <el-menu-item index="4-2">查看客户信息</el-menu-item>
              <el-menu-item index="4-3">客户信息报表</el-menu-item>
          </el-submenu>
        </el-menu>
      </el-aside>
</template>

<script>
    export default {
        name: "SideBar",
        methods: {
          handleIconClick(ev) {
            console.log(ev);
          },
          handleSelect(key, keyPath){
            switch(key){
              case '1-1':
                this.$router.push('/');
                break;
              case '1-2':
                this.$router.push('/learning');
                break;
              case '1-3':
                this.$router.push('/showStudents');
                break;
              case '1-4':
                this.$router.push('/newstudent');
                break;
              case '2-1':
                this.$router.push('/chart');
                break;
              case '2-2':
                this.$router.push('/barchart');
                break;
              case '3-1':
                this.$router.push('/linux');
                break;
              case '4-1':
                this.$router.push('/customer/new');
                break;
              case '4-2':
                this.$router.push('/customer/list');
                break;
              case '4-3':
                this.$router.push('/barchart');
                break;
              }
            },
         }
        }
</script>

<style scoped>

</style>
